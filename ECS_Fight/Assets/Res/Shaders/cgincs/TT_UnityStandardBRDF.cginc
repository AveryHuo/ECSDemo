...
#include "TT_UnityLightingCommon.cginc"
...
half4 BRDF1_Unity_PBS(half3 diffColor, half3 specColor, half oneMinusReflectivity, half smoothness,
	...
	UnityLight light, UnityIndirect gi, half4 emiss, half4 mix, half atten)
{
...
#   endif
	...
	half3 color = diffColor * (gi.diffuse + light.color*(atten * diffuseTerm))
		+ specularTerm * 0.7*atten * FresnelTerm(specColor, lh)*emiss.g
...
//Skin
#if _SKIN
inline fixed4 SKIN_BRDF_PBS(float3 posWorld, half3 diffColor, half3 specColor, half oneMinusReflectivity, half smoothness,
	float3 normal, float3 viewDir, UnityLight light, UnityIndirect gi, half4 mix, half4 emiss, half atten , half Specstr , half SPstr , half GLstr,half3 ambient, half3 mix2, half Flatstr)
{
...
#define OneOnLN2_x6 8.656170
	...
	half dotNL = dot(normal, light.dir);
	...
	//float curvature = saturate(length(fwidth(normal)) / length(fwidth(posWorld)));
	...
	//float3 H = normalize(light.dir);
	//float transDot = saturate(dot(viewDir, -H));
	//half3 lightScattering = transDot;

	//	#endif
	//	Final composition
	float2 brdfUV;
	
	half difcombine = min(atten, dotNL*0.5 + 0.5);
	half difcombine2 = pow(saturate(difcombine),0.454545);


	brdfUV.x = (difcombine-0.4)*1.6666;
	brdfUV.y = 0.5;
	half3 brdf =(atten*0.5+0.5) * tex2D(_BRDFTex, brdfUV).rgb;
	half4 c = 0;
	half3 IBL = atten*gi.specular * F_L*Specstr;
	half3 SPE = clamp(D *emiss.rgb*0.21 *SPstr, 0, 2 * mix2.g * Flatstr);
	c.rgb = diffColor*light.color*lerp(difcombine2, pow(brdf,0.5), mix.b)// diffuse
		+ ambient * diffColor//ambient add
		+ SPE  // direct specular
		+ IBL
...
#endif
#if _SKINLOD1
inline fixed4 SKINLOD1_BRDF_PBS(float3 posWorld, half3 diffColor, half3 specColor, half oneMinusReflectivity, half smoothness,
	float3 normal, float3 viewDir, UnityLight light, UnityIndirect gi, half4 mix, half atten, half Specstr, half3 ambient)
{
...
#endif
#if _SKINLOW
inline fixed4 SKINLOW_BRDF_PBS(float3 posWorld, half3 diffColor, half3 specColor, half oneMinusReflectivity, half smoothness,
	float3 normal, float3 viewDir, UnityLight light, UnityIndirect gi, half4 mix, half4 emiss, half atten, half Specstr, half3 ambient)
{
	//half curvature = sssTex.r*_CurvatureScale;
	//half thickness = 1-sssTex.g;
...
#define OneOnLN2_x6 8.656170
	...
	//	Cook Torrrance
	//	from The Order 1886 // http://blog.selfshadow.com/publications/s2013-shading-course/rad/s2013_pbs_rad_notes.pdf
	half alpha = 1 - emiss.a; // alpha is roughness
	alpha *= alpha;
	half alpha2 = alpha * alpha;

	//	Specular Normal Distribution Function: GGX Trowbridge Reitz
	half denominator = (dotNH *dotNH)* (alpha2 - 1.f) + 1.f;
	half D = alpha2 / (denominator * denominator);
	//	Geometric Shadowing: Smith
	// B. Karis, http://graphicrants.blogspot.se/2013/08/specular-brdf-reference.html
	//half G_L = dotNL + sqrt((dotNL - dotNL * alpha) * dotNL + alpha);
	//half G_V = dotNV + sqrt((dotNV - dotNV * alpha) * dotNV + alpha);
	...
	//half a004 = min(r.x * r.x, exp2(-9.28 * dotNV)) * r.x + r.y;
	//half2 AB = half2(-1.04, 1.04) * a004 + r.zw;
	//half3 F_L = specColor * 0.8;

	
	float2 brdfUV;
	half difcombine = dotNL*0.5 + 0.5+ transVdotL*light.color;
	half difcombine2 = pow(saturate(difcombine), 0.4545);
	...
	half4 c = 0;
	c.rgb = diffColor*light.color*lerp(difcombine2, pow(brdf, 0.454545), mix.b)// diffuse
		+ ambient * diffColor//ambient add
		+ D *dotNL*emiss.rgb*0.3* light.color  // direct specular
...
#endif
#if _SKINLOWLOD2
inline fixed4 SKINLOWLOD2_BRDF_PBS(float3 posWorld, half3 diffColor, half3 specColor, half oneMinusReflectivity, half smoothness,
	float3 normal, float3 viewDir, UnityLight light, UnityIndirect gi, half atten, half3 ambient)
{
	//half curvature = sssTex.r*_CurvatureScale;
	//half thickness = 1-sssTex.g;
...
#define OneOnLN2_x6 8.656170
	...
	half4 c = 0;
	c.rgb = diffColor*light.color// diffuse
		//+ ambient * diffColor
		; 
	c.a = 1;
...
#endif
#if _SKINLOWGRAY
inline fixed4 SKINLOWGRAY_BRDF_PBS(float3 posWorld, half3 diffColor, half3 specColor, half oneMinusReflectivity, half smoothness,
	float3 normal, float3 viewDir, UnityLight light, UnityIndirect gi,  half4 emiss, half atten, half Specstr, half3 ambient)
{
	//half curvature = sssTex.r*_CurvatureScale;
	//half thickness = 1-sssTex.g;
...
#define OneOnLN2_x6 8.656170
	...
	float2 brdfUV;
	half difcombine = dotNL*0.6 + 0.4 + transVdotL*light.color;
	half difcombine2 = pow(saturate(difcombine), 0.454545);


	/*brdfUV.x = difcombine*0.83 + 0.17;
	brdfUV.y = 0.5;
	half3 brdf = tex2D(_BRDFTex, brdfUV).rgb;*/
	half4 c = 0;
	c.rgb = diffColor*light.color*difcombine2// diffuse
		+ ambient * diffColor//ambient add
		//+ D *dotNL*(emiss.r + emiss.g + emiss.b)*0.1* light.color  // direct specular
...
#endif
#if _SKINLOWGRAYLOD2
inline fixed4 SKINLOWGRAYLOD2_BRDF_PBS(float3 posWorld, half3 diffColor, half3 specColor, half oneMinusReflectivity, half smoothness,
	float3 normal, float3 viewDir, UnityLight light, UnityIndirect gi, half atten, half3 ambient)
{
	//half curvature = sssTex.r*_CurvatureScale;
	//half thickness = 1-sssTex.g;
...
#endif
#if _REFLECT
inline fixed4 REFLECT_BRDF_PBS(float3 posWorld, half3 diffColor, half3 specColor, half oneMinusReflectivity, half smoothness,
	float3 normal, float3 viewDir, UnityLight light, UnityIndirect gi, half4 mix, half4 emiss, half atten, half Specstr, half SPstr, half GLstr, half4 basecolor, half3 ambient, half3 mix2, half Flatstr)
{
	//half curvature = sssTex.r*_CurvatureScale;
	//half thickness = 1-sssTex.g;
...
#define OneOnLN2_x6 8.656170
	...
	//	Specular Normal Distribution Function: GGX Trowbridge Reitz
	float denominator = (dotNH *dotNH)* (alpha2 - 1.f) + 1.f;
	float D = alpha2 / (denominator * denominator);
	const half4 c0 = { -1, -0.0275, -0.572, 0.022 };
	const half4 c1 = { 1, 0.0425, 1.04, -0.04 };
	half4 r = (1 - oneMinusRoughness) * c0 + c1;
	...
	half difcombine = dotNL*0.5 + 0.5;
	half difcombine2 = pow(saturate(difcombine), 0.454545);

	half4 c = 0;
	half3 IBL = 1.1662*gi.specular * F_L*Specstr;
	half3 SPE = clamp(D *emiss.rgb*0.21*SPstr, 0, 2 * mix2.g * Flatstr);
	c.rgb = diffColor*light.color*difcombine2// diffuse
		+ ambient * diffColor//ambient add
		+ SPE // direct specular
	...
	c.a = saturate(basecolor.a + SPE + IBL);
...
#endif
#if _REFLECTLOD1
inline fixed4 REFLECTLOD1_BRDF_PBS(float3 posWorld, half3 diffColor, half3 specColor, half oneMinusReflectivity, half smoothness,
	float3 normal, float3 viewDir, UnityLight light, UnityIndirect gi, half4 mix, half atten, half Specstr, half4 basecolor, half3 ambient)
{
	//half curvature = sssTex.r*_CurvatureScale;
	//half thickness = 1-sssTex.g;
...
#endif
//#if _CAT
//inline fixed4 CAT_BRDF_PBS(float3 posWorld, half3 diffColor, half3 specColor, half oneMinusReflectivity, half smoothness,
//	float3 normal, float3 viewDir, UnityLight light, UnityIndirect gi, half4 mix, half4 emiss, half atten)
//{
//	//half curvature = sssTex.r*_CurvatureScale;
//	//half thickness = 1-sssTex.g;
//
...
//	//	Specular Normal Distribution Function: GGX Trowbridge Reitz
//	half denominator = (dotNH * dotNH) * (alpha2 - 1.f) + 1.f;
//	half D = max(alpha2 / (denominator * denominator), 0);
//	//	Geometric Shadowing: Smith
//	// B. Karis, http://graphicrants.blogspot.se/2013/08/specular-brdf-reference.html
//
//	half G_L = dotNL + sqrt((dotNL - dotNL * alpha) * dotNL + alpha);
...
//
//
//	half3 difcombine = atten*dotNL;
//	half DP1 = mix.b;
//	half DP2 = 1 - mix.b;
//	half4 c = 0;
//	difcombine = difcombine * DP2 + DP1;
//	c.rgb = diffColor*difcombine*light.color// diffuse
//		+ gi.diffuse * diffColor
//		+ D * light.color * atten*dotNL*emiss.rgb*0.3// direct specular
//		+ gi.specular * F_L // * FresnelSchlickWithRoughness;	
//							//emiss//
//		;
//	c.a = mix.a;
//
...
//
//#if _CATLOW
//inline fixed4 CATLOW_BRDF_PBS(float3 posWorld, half3 diffColor, half3 specColor, half oneMinusReflectivity, half smoothness,
//	float3 normal, float3 viewDir, UnityLight light, UnityIndirect gi, half4 mix, half atten)
//{
//	//half curvature = sssTex.r*_CurvatureScale;
//	//half thickness = 1-sssTex.g;
//
...
#if _HAIR  
inline half AnisoDCore(half smoothness, half3 normalWorld, half3 tangentWorld, half3 halfDir, half nh, half D, half gloss, half spec, half mask, half4 mix, half atten, half Specstr, half3 ambient)
{
...
inline half AnisoD(half smoothness, half3 normalWorld, half3 tangentWorld, half3 halfDir, half nh, half D, half2 anisoCtrl, half4 mix, half atten, half Specstr, half3 ambient)
{
	half jitter = anisoCtrl.r;
	half mask = mix.b;

	half3 tangentWorld1 = JitterTangent(tangentWorld, normalWorld, jitter + _TangentShift1);
	half AnisoDLow = AnisoDCore(smoothness, normalWorld, tangentWorld1, halfDir, nh, D, _AnisoGloss1, _AnisoSpec1, mask, mix, atten, Specstr, ambient);

	half3 tangentWorld2 = JitterTangent(tangentWorld, normalWorld, jitter + _TangentShift2);
	half AnisoDHigh = AnisoDCore(smoothness, normalWorld, tangentWorld2, halfDir, nh, D, _AnisoGloss2, _AnisoSpec2, mask, mix, atten, Specstr, ambient);

	return /*AnisoDLow + */AnisoDHigh;//save performance
}

half4 HAIR_BRDF_PBS(half3 diffColor, half3 specColor, half oneMinusReflectivity, half smoothness,
	float3 normal, float3 viewDir,
	UnityLight light, UnityIndirect gi, float3 tangentWorld = float3(0, 0, 1), half2 anisoCtrl = half2(1, 1), half4 mix = half4(1, 1, 1, 1), half atten = 0, half Specstr = 1, half3 ambient = half3(0.09, 0.09,0.09))
{
	float perceptualRoughness = SmoothnessToPerceptualRoughness(smoothness);
	float3 halfDir = Unity_SafeNormalize(float3(light.dir) + viewDir);
...
#define UNITY_HANDLE_CORRECTLY_NEGATIVE_NDOTV 0
	...
	D = AnisoD(smoothness, normal, tangentWorld, halfDir, nh, D, anisoCtrl, mix, atten, Specstr, ambient);
...
#   endif
	...
	half transVdotL = clamp((dot(viewDir, -light.dir) + light.color*1.3), 0, 0.7);
	half grazingTerm = saturate(smoothness + (1 - oneMinusReflectivity));
	half3 color = ambient * diffColor//ambient add
		+ diffColor *light.color*(atten * diffuseTerm*0.6 + 0.4 )
		+ mix.b*_AnisoSpec2*specularTerm *light.color *atten* FresnelTerm(specColor, lh)
		+ Specstr*surfaceReduction * gi.specular * FresnelLerp(specColor, grazingTerm, nv);

...
#include "TT_HLSLSupport.cginc"
#include "TT_UnityShadowLibrary.cginc"
...
// ---- Screen space direction light shadows helpers (any version)
        ...
        inline fixed unitySampleShadow (unityShadowCoord4 shadowCoord)
        {
			...
			//shadow = _LightShadowData.r + shadow * (1 - _LightShadowData.r);
				...
				fixed4 shadows;
				fixed4 xshadows;
				shadows.x = UNITY_SAMPLE_SHADOW(_ShadowMapTexture, shadowCoord + half4(0, 0.001, 0, 0)).r;
				shadows.y = UNITY_SAMPLE_SHADOW(_ShadowMapTexture, shadowCoord + half4(0.001, 0, 0, 0)).r;
				shadows.z = UNITY_SAMPLE_SHADOW(_ShadowMapTexture, shadowCoord + half4(0, -0.001, 0, 0)).r;
				shadows.w = UNITY_SAMPLE_SHADOW(_ShadowMapTexture, shadowCoord + half4(-0.001, 0, 0, 0)).r;
				shadows = _LightShadowData.rrrr + shadows * (1 - _LightShadowData.rrrr);
				xshadows.x = UNITY_SAMPLE_SHADOW(_ShadowMapTexture, shadowCoord + half4(0, 0.002, 0, 0)).r;
				xshadows.y = UNITY_SAMPLE_SHADOW(_ShadowMapTexture, shadowCoord + half4(0.002, 0, 0, 0)).r;
				xshadows.z = UNITY_SAMPLE_SHADOW(_ShadowMapTexture, shadowCoord + half4(0, -0.002, 0, 0)).r;
				xshadows.w = UNITY_SAMPLE_SHADOW(_ShadowMapTexture, shadowCoord + half4(-0.002, 0, 0, 0)).r;
				xshadows = _LightShadowData.rrrr + xshadows * (1 - _LightShadowData.rrrr);
				shadow = dot(shadows, 0.25);
				xshadow = dot(xshadows, 0.25);
				shadow = (shadow+xshadow)*0.5;

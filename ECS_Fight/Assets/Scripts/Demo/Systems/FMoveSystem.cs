...
namespace Test
{
    public class FMoveSystem : JobComponentSystem
    {
        [BurstCompile]
        private struct MoveJob : IJobChunk
        {
            ...
            public void Execute(ArchetypeChunk chunk, int chunkIndex, int firstEntityIndex)
            {
                ...
                var chunkTrans = chunk.GetNativeArray(PositionComponentType);
                ...
                for (var i = 0; i < chunk.Count; i++)
                {
                   ...
                   + trans.Value.x * waveData.xOffset + trans.Value.y * waveData.yOffset);
                    ...
                    //};
                    chunkTrans[i] = new PositionComponent
                    {
                        Value = new float3(trans.Value.x, trans.Value.y, zPos)
                    };
                }
        ...
        {
            m_Group = GetEntityQuery(ComponentType.ReadWrite<PositionComponent>(),
        ...
        protected override JobHandle OnUpdate(JobHandle inputDeps)
        {
            var job = new MoveJob()
            {
                PositionComponentType = GetArchetypeChunkComponentType<PositionComponent>(false),
